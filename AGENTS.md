...
Forked projects sync template updates via `scripts/template-sync/cli.mjs`; `template-sync.json` is the ownership map. `overwrite` paths (`.agents/`, `.github/workflows/`, `AGENTS.md`, `docs/ARCHITECTURE.md`, `docs/BOOTSTRAP_PROMPT.md`, `docs/QUOTA.md`, `docs/RUNBOOK_RESTORE.md`, `scripts/`, root configs, `template-sync.json` itself) are template-owned — never edit them in a project; `bun run template-gate` fails on drift. `merge` paths (`apps/`, `packages/`, `package.json`, `README.md`, `CONTEXT.md`) inherit changes — extend project code there. Unlisted paths are project-owned.
