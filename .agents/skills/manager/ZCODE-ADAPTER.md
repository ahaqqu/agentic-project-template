...
Mechanics per ZCode's native behavior. Evidence class: spawn/resume/result mechanics verified by live probes on the installed client (2026-08-30, this repo); pin resolution is **config-dependent, not verified by the probes** — see Model routing below.
...
- **Model routing:** handled by ZCode — the `model:` frontmatter ref in each role file resolves through the client's provider config (`~/.zcode/v2/config.json`, `provider` object). A pin that does not resolve fails the spawn with "Model provider is not configured: <id>"; fix it in the client's provider settings, not the role file. Pin changes reach new spawns only after a client restart. Override order: `~/.zcode/agents/<role>.md` (user) → this project's `.zcode/agents/`. A model-pinned dispatch keeps `SendMessage` continuation.
