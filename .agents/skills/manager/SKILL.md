...
Every implementer-class dispatch prompt also carries a **hard context budget** and the escape hatch. The defaults live in the role registry (`.zcode/agents/README.md`: ~150k billed input tokens or ~150 requests per phase, whichever is hit first); a dispatch may override them tighter. The prompt must state the escape hatch: when billed input or request count passes the budget, A stops expanding context, makes a checkpoint commit, pushes the branch, and hands off — either to a fresh scoped context carrying the last checkpoint (the respawn intake check applies), or back to you through A's normal report channel — instead of continuing in a bloated one. A budget handoff is compliance, not failure; silently continuing past the budget is the anti-pattern.
